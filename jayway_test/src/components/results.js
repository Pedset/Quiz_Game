...
export default function Results(){
    ...
    useEffect(()=>{
        ...
        answerLog.forEach(element => {
           ...
           if(element.answeredTime < fastestTime.time && element.passed != null) fastestTime = {time : element.answeredTime, question : qAndA[element.questionNumber].question , img : qAndA[element.questionNumber].img ? (qAndA[element.questionNumber].img) : null};
           if(element.answeredTime > slowestTime.time && element.passed != null) slowestTime = {time : element.answeredTime, question : qAndA[element.questionNumber].question , img : qAndA[element.questionNumber].img ? (qAndA[element.questionNumber].img) : null};
        });
        ...
        if (answerTimes !== undefined && answerTimes.length !== 0) setAvgTime(answerTimes.reduce((a, b) => a + b) / answerTimes.length); 
    ...
    return (
       <Fragment>
           ...
           {fastestTime.question ? (<p>{fastestTime.img ? (<img className="q_img" alt="question_img" src={fastestTime.img}></img>) : null }</p>) : null}
           {fastestTime.question ? (<hr></hr>) : null}
           </div>
           {slowestTime.question ? (<p>Your slowest question answered was {slowestTime.time} sec for the question: </p>) : null}
           <div className="question-container">
           {slowestTime.question ? (<p>{slowestTime.question}</p>) : null}
           {slowestTime.question ? (<p>{slowestTime.img ? (<img className="q_img" alt="question_img" src={slowestTime.img}></img> ) : null }</p>): null}
           {slowestTime.question ? (<hr></hr>) : null}
           </div>
